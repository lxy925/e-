	...
	<view class="container">
				...
				<view :class="['message', isSelfMessage(item) ? 'self' : 'other']">
					...
					<view class="msg-content" @longpress="handleLongPress">
						...
						<template v-else-if="item.message_type === 'image'">
							<image :src="item.content" mode="widthFix" class="msg-image"
								@click="previewImage(item.content)" :style="{ maxWidth: '400rpx' }"></image>
						</template>
							...
							<view class="location-message" @click="openLocation(item.content)">
								<view class="location-content">
									<view class="location-info">
										<text
											class="location-name">{{JSON.parse(item.content).name || JSON.parse(item.content).address}}</text>
										<text class="location-address">{{JSON.parse(item.content).address}}</text>
									</view>
								</view>
								<view class="location-map">
									<map :latitude="JSON.parse(item.content).latitude"
										:longitude="JSON.parse(item.content).longitude" :markers="[{
                      ...
                      width: 32,
                      height: 32
                    }]" scale="16" style="width: 100%; height: 200rpx;"></map>
								</view>
							</view>
						</template>
					</view>
		...
		<view class="footer">
			<view class="input-area">
				...
				<input type="text" v-model="messageText" placeholder="请输入内容" @focus="onInputFocus" />
		...
		<view class="video-fullscreen" v-if="isFullscreen">
			<video :id="'fullscreen-video'" :src="currentVideoUrl" class="fullscreen-video" controls autoplay
				:show-fullscreen-btn="false" :show-center-play-btn="false" :enable-progress-gesture="true"
				@ended="exitFullscreen"></video>
		...
		<view class="review-modal" v-if="showReviewModal">
			<view class="review-content">
				...
				<view class="rating-section">
					...
					<view class="stars">
						<text v-for="i in 5" :key="i" class="star" :class="{ active: i <= reviewRating }"
							@click="setRating(i)">★</text>
					</view>
				</view>
				<view class="content-section">
					<textarea v-model="reviewContent" placeholder="请输入您的评价内容" class="review-textarea"></textarea>
				</view>
				<view class="image-upload-section">
					<view class="image-list">
						<view v-for="(image, index) in reviewImages" :key="index" class="image-item">
		...
		<view class="summary-modal" v-if="showSummaryModal">
			<view class="summary-content">
				...
				<view class="content-section">
					<textarea v-model="summaryContent" placeholder="请输入就诊总结内容" class="summary-textarea"></textarea>
				</view>
				<view class="image-upload-section">
					<view class="image-list">
						<view v-for="(image, index) in summaryImages" :key="index" class="image-item">
							<image :src="image" mode="aspectFill"></image>
			...
			<view class="check-in-content">
				...
				<view class="check-in-body">
					...
					<view class="image-section">
						...
						<view class="image-list">
							<view v-for="(image, index) in checkInImages" :key="index" class="image-item">
								<image :src="image" mode="aspectFill"></image>
					...
					<view class="description-section">
						...
						<textarea v-model="checkInDescription" placeholder="请输入备注说明"
		...
		methods: {
			...
			async getChatPartner() {
				try {
					...
					const {
						result
					} = await uniCloud.database().collection('users').where({
						user_id: partnerId
					}).get();
								...
								const {
									...
									data: {
										...
										data: {
											userId: this.userId,
											chatPartnerId: this.chatPartner.user_id,
											lastMessageTime: this.messageList.length > 0 ? this
												.messageList[this.messageList.length - 1].time :
												'0',
											sender_type: this.userType
										}
									}
								...
								if (result.code === 200 && result.data && result.data.length > 0) {
									...
									const newMessages = result.data.filter(newMsg =>
										!this.messageList.some(existingMsg =>
											...
											(existingMsg.status === 'sending' && existingMsg.time ===
												newMsg.time)
										)
									);
