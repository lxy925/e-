    ...
    <view class="object_info">
	 ...
	 <!-- 服务对象照片 -->
      <view class="photo-container">
		...
		<image :src="photoUrl||'../../static/images/mine/avatar.png'" class="default-upload-photo"  mode="aspectFill" @click="previewImage(photoUrl)" />
		</button>
		</view>
      </view>
      ...
      <view class="input_group">
           ...
           <view class="gender-select-container">
             <button class="gender-select" :class="{ active: selectedGender === '男' }" @tap="selectGender('男')">男</button>
             <button class="gender-select" :class="{ active: selectedGender === '女' }" @tap="selectGender('女')">女</button>
           </view>
         </view>
      <view class="input_group">
...
export default {
  ...
  methods: {
    ...
    submitData() {
      ...
      if (!this.name || !this.selectedGender || !this.age || !this.phone || !this.relationship || !this.userInfo.user_id||this.address) {
        uni.showToast({
          title: '所有字段都是必填的',
          icon: 'none'
        });
        ...
        success: res => {
          ...
          uni.request({
            ...
            data: {
              ...
              secret: 'YOUR_SECRET', // 替换为你的小程序秘钥secret
...
.photo-container {
  display: flex;
  justify-content: center; /* 水平居中 */
  margin: 30rpx 0rpx 0 40rpx; /* 上下间距 */
  width: 100%; /* 图片宽度 */
  height: 200rpx; /* 图片高度 */
}
...
@import './static/iconfont/iconfont.wxss'; /* 引入图标字体样式 */
</style>