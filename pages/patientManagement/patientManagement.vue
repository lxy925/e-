  ...
  <view class="page">
    ...
    <view class="content">
      <view v-if="patients.length > 0" class="patients-list">
        <view v-for="(patient, index) in patients" :key="index" class="patient-item" @click="selectPatient(patient)">
          ...
          <text class="patient-relationship">关系: <text class="highlight">{{ patient.relationship }}</text></text>
          <text class="patient-phone">电话: <text class="highlight">{{ patient.phone }}</text></text>
          <text class="patient-medical-info">医疗信息: <text class="highlight">{{ patient.medicalInfo }}</text></text>
          <view v-if="patient.uploadedImages.length > 0" class="uploaded-images">
            <text>上传的图片:</text>
            <view class="image-list">
              <view v-for="(image, imgIndex) in patient.uploadedImages" :key="imgIndex" class="image-item">
                <image :src="image" mode="aspectFill" class="image" @click="previewImage(patient.uploadedImages, imgIndex)" />
              </view>
            </view>
          </view>
        ...
        success: res => {
          ...
          uni.request({
            ...
            success: (res) => {
              ...
              if (res.data.openid) {
                this.fetchPatients(res.data.openid); // 直接传递openid给fetchPatients
              } else {
                uni.showToast({
                  title: '未获取到openid',
                  icon: 'none'
...
.patient-item {
  padding: 15rpx;
  margin-bottom: 15rpx; /* 添加下边距以创建间隔 */
  border: 1rpx solid #e0e0e0; /* 可选：添加边框以增强视觉效果 */
...
.patient-photo {
  width: 110rpx;
  height: 110rpx;
  border-radius: 80rpx; /* 圆形照片 */
  margin-right: 15rpx; /* 照片与文字之间的间距 */
}
...
.patient-medical-info {
  ...
  color: #666;
  font-size: 25rpx;
  margin-top: 5rpx; /* 每行之间的间距 */
}

.highlight {
  font-weight: bold; /* 强调信息 */
  color: #007aff; /* 使用蓝色强调 */
}
