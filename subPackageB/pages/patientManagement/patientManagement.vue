<template>
	<view class="page-container">
		...
		<scroll-view scroll-y class="main-scroll" @scroll="handleScroll" :style="{ 
        ...
        height: 'calc(100vh - ' + navHeight + 'px)'
			...
			<view class="content-container">
				...
				<view v-if="patients.length > 0" class="patients-list">
					<view v-for="(patient, index) in patients" :key="index" class="patient-card"
						@click="selectPatient(patient)" @touchstart="touchStart(index)" @touchend="touchEnd(index)"
						:class="{ 'touched': activeIndex === index }">
						<view class="card-header">
							<image v-if="patient.photo" :src="patient.photo" mode="aspectFill" class="patient-avatar" />
							<image v-else src="../../static/images/default-avatar.png" mode="aspectFill"
						...
						<view class="card-footer">
							<view class="action-btns">
								...
								<view class="delete-btn" @click.stop="deletePatient(patient._id, index)">
	...
	export default {
		...
		methods: {
			...
			deletePatient(patientId, index) {
				...
				uni.showModal({
					title: '确认删除',
					content: '删除后就诊人信息将无法恢复，是否继续？',
					confirmText: '删除',
					cancelText: '取消',
					confirmColor: '#FF4C4C', // 确认按钮红色（警示色）
					success: (res) => {
						// 2. 用户点击“删除”时执行逻辑
						if (res.confirm) {
							uni.showLoading({
							...
							// 3. 调用云函数删除就诊人（需与后端云函数名、参数对齐）
							uniCloud.callFunction({
								name: 'deletePatient', // 云函数名：需与后端定义一致
								data: {
									server_id: patientId, // 传递要删除的就诊人ID
