	...
	<view class="app">
		...
		<view class="page-content">
			...
			<radio-group @change="radioChange" style="display: flex;margin: 10rpx 0;">
				<label style="flex: 1;">
					<radio value="wxpay" :checked="form1.provider == 'wxpay'" /> <text>微信</text>
				</label>
				<label style="flex: 1;">
					<radio value="alipay" :checked="form1.provider == 'alipay'" /> <text>支付宝</text>
				</label>
			</radio-group>
			<!-- #endif -->
			...
			<button class="button" v-if="h5Env === 'h5-weixin'" type="primary"
				@click="getWeiXinJsCode('snsapi_base')">公众号获取openid示例</button>
			...
			createPayment(obj = {}) {
				...
				that.$refs.vkPay.createPayment({
					...
					create: res => {
						...
						this.form1.out_trade_no = res
							.out_trade_no; // 如果订单号是云端生成的，这里可以拿到订单号，此处如果return false; 则不再执行后续逻辑
					},
					// 支付成功回调
					success: res => {
