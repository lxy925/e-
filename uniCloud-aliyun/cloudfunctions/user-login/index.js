...
exports.main = async (event, context) => {
  const { code, encryptedData, iv, nickName, realName, idCard, phoneNumber, avatar } = event;
  console.log("code是:", code);

  // 1. 获取openid
  const wxUrl = `https://api.weixin.qq.com/sns/jscode2session?appid=${APPID}&secret=${SECRET}&js_code=${code}`;
  let wxRes;
  try {
    wxRes = await uniCloud.httpclient.request(wxUrl, { dataType: 'json' });
    console.log('微信接口响应:', wxRes.data);
  } catch (err) {
    return { code: -2, msg: '请求微信接口失败', error: err };
  }
  ...
  if (encryptedData) {
    try {
     ...
     return {code:200,data:decryptedPhone = decryptedData.phoneNumber || decryptedData.purePhoneNumber};
    } catch (err) {
      return { code: -3, msg: '手机号解密失败', error: err.message };
    }
  }

  // 3. 数据校验
  if (!validateIdCard(idCard)) {
    return { code: -1, msg: '身份证格式错误' };
  }
  ...
  // 检查用户是否已存在
  const userRes = await usersCollection.where({ user_id: openid }).get();
  ...
  return { 
    ...
    data: {
      token,
      refreshToken,
      expiresIn: 900, // accessToken有效期(秒)
      userInfo: {
        user_id: token,
        avatar,
        nickName,
